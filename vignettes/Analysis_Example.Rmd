...
total_Annotation <- 
  annotation_table %>% 
  dplyr::mutate(Barcode = as.factor(.$Barcode)) %>% 
  mutate(Gene_ID = NA, Sequence = NA, Library = NA)
```
...
```{r Create_Object, message=FALSE, warning=FALSE}
...
```{r  distribution_mapped_reads_boxplot, message=FALSE, warning=FALSE}
...
### Density plot
```{r  distribution_mapped_reads_density, message=FALSE, warning=FALSE}
plot <- distribution_mapped_reads(
...
#### For Sample
```{r  Plot_MDS_Sample, message=FALSE, warning=FALSE}
plot_MDS(screenR_Object = object) 
```


#### For Treatment
```{r  Plot_MDS_Treatment, message=FALSE, warning=FALSE}
...
#### For Day
```{r  Plot_MDS_Day, message=FALSE, warning=FALSE}
...
```{r  Z_score_hit, message=FALSE, warning=FALSE}
zscore_hit_TRT <- list(
...
```{r Common_Hit,  message=FALSE, warning=FALSE}
