...
#'                            for the the control
#' @return The screenR_object with the slope for the treatment and the control
#'         for each gene
#' @export

filter_by_slope <- function(screenR_Object, genes, group_var_treatment,
                            group_var_control,
                            slope_control = NULL, slope_treatment){
  ...
  data <- dplyr::filter(data,
                        .data$slope_treatment <= slope_treatment)

  screenR_Object@data_table <- data
...
compute_slope <-  function(screenR_Object, genes, group_var){
  ...
  # in order to perform the linear regression as samples are a time
  # series the ordered Sample are encoded as number
  data <- dplyr::group_by(data, .data$Gene)
  data <- dplyr::mutate(data, encode = 1:n())
  data <- dplyr::ungroup(data)

  data <- dplyr::mutate(dplyr::nest_by(data, .data$Gene),
                        Slope = lm(Frequency ~ encode,
                                   data = data)$coefficients["encode"])
