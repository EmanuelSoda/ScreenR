...
find_common_hit <- function(hit_zscore, hit_camera, hit_roast, common_in = 3) {
    ...
    if (common_in == 3) {
        ...
        # We use the function Reduce to apply the intersect to
        # the list and obtain a vector
...
#' @export
plot_common_hit <- function(hit_zscore, hit_camera, hit_roast,
                            alpha = 0.5,
                            stroke_size = 0.5,
                            set_name_size = 4,
                            text_color = "black",
    ...
    hit_list <-
        list("Z-score Hits" = unique(hit_zscore$Gene),
