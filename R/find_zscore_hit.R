...
find_zscore_hit <- function(table_treate_vs_control, number_barcode = 6,
    ...
    if (metric == "median") {
        hit_table <- table_treate_vs_control %>%
            # Filtering of the gene that have the Z-score under
            # the median of the Z-scores
        dplyr::filter(.data$Zscore < median(.data$Zscore)) %>%
            dplyr::group_by(.data$Gene) %>%
            dplyr::summarise(numberOfBarcode = n()) %>%
            # Take only the gene that have 6 barcode under the
            # median of the Z-scores
        ...
        # Take only the gene that have 6 barcode under the median
        # of the Z-scores
        dplyr::filter(.data$numberOfBarcode > number_barcode)
    }
